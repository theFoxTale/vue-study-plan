...
Следующий теоретический блок Module 1 — **директивы**:

- [`v-bind`](./07-v-bind.md)
- затем: [`v-if`](./08-v-if.md) · [`v-show`](./09-v-show.md) · [`v-for`](./10-v-for.md) · [`v-model`](./11-v-model.md) · [`v-on`](./12-v-on.md)

После событий логично закрепить привязку атрибутов и условный/списочный рендеринг.
